...
const DocCommentsSchema = Type.Object({
    ...
    file_token: Type.String({
        description: "云文档token(可从文档URL获取)",
    }),
    ...
    api.registerTool({
        ...
        async execute(_toolCallId, params) {
            ...
            try {
                ...
                // Action: list - 获取评论列表
                if (p.action === "list") {
                    log.info(`doc_comments.list: file_token="${p.file_token}", file_type=${p.file_type}`);
                    ...
                    // 组装评论和完整回复
                    const assembledItems = await assembleCommentsWithReplies(client, p.file_token, p.file_type, items, userIdType, log);
                    return json({
                ...
                if (p.action === "create") {
                    ...
                    log.info(`doc_comments.create: file_token="${p.file_token}", elements=${p.elements.length}`);
